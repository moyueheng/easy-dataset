/**
 * 构建 GA 提示词
 * @param {Array} gaPairs GA 组合列表
 * @param {Object} activeGaPair 当前激活的 GA 组合
 * @returns {String} 构建的 GA 提示词
 */
function buildGaPrompt(gaPairs = [], activeGaPair = null) {
...
请确保：
1. 问题风格符合「${activeGaPair.genre}」的特点和表达习惯
2. 问题深度和表达方式适合「${activeGaPair.audience}」的知识背景
...
module.exports = function getQuestionPrompt({
  ...
  // 构建GA pairs相关的提示词
  const gaPrompt = buildGaPrompt(gaPairs, activeGaPair);
