...
export async function getQuestionsForTree(projectId, input, isDistill = false) {
  try {
    ...
    console.log(`[getQuestionsForTree] 查询到 ${data.length} 个问题`);
    return data;
  } catch (error) {
...
export async function getQuestionsByTag(projectId, tag, input, isDistill = false) {
  try {
    console.log('[getQuestionsByTag] 参数:', { projectId, tag, input, isDistill });
    ...
    console.log(`[getQuestionsByTag] 查询到 ${data.length} 个问题`);
