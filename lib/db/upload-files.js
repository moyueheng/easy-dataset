...
export async function readFileContent(fileId) {
  try {
    const fileInfo = await db.uploadFiles.findUnique({
      where: { id: fileId }
    });
    ...
    let filePath;
    if (fileInfo.fileExt !== '.md') {
      // 对于非markdown文件，查找对应的.md文件
      filePath = path.join(fileInfo.path, fileInfo.fileName.replace(/\.[^/.]+$/, '.md'));
    } else {
      filePath = path.join(fileInfo.path, fileInfo.fileName);
    }
 ...
 * 获取项目下的所有已上传文件（别名函数）
 */
export async function getUploadedFilesByProjectId(projectId) {
  return await getUploadFilesByProjectId(projectId);
}
