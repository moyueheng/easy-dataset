...
export async function processPdfProcessingTask(task) {
  try {
    ...
    if (!projectId || !file) {
      ...
      throw new Error('missing required parameters: projectId or file');
    }
    ...
    await updateTask(task.id, {
      ...
      detail: `${file.fileName} processing ${strategy} strategy`,
      startTime: new Date()
    });
    ...
    try {
      ...
      console.log(`${strategy} strategy processed successfully: ${file.fileName}`);
      // 更新任务进度
      await updateTask(task.id, {
        status: 1,
        detail: `${strategy} strategy processed successfully`+ JSON.stringify({
          ...result.data,
          strategy,
          status: 'completed'
        }),
      ...
      console.error(`processing ${file.fileName} failed:`, error);

      // 更新任务状态为失败
      await updateTask(task.id, {
        status: 2, // 2表示失败
        completedCount: 0,
        detail: `${file.fileName},${strategy} strategy processing failed: ${error.message}`,
        endTime: new Date()
      });
    ...
    await updateTask(task.id, {
      status: 2, // 2表示失败
      detail: `rocessing failed: ${error.message}`,
      endTime: new Date()
    });
  }
}
