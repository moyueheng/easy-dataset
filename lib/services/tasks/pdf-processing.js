...
import { processPdf } from '@/lib/file/file-process/pdf';
  ...
  try {
    ...
    for (const file of fileList) {
      try {
        ...
        taskMessage.current.processedPage = 0; // 重置当前处理页数
        taskMessage.current.totalPage = file.pageCount || 0; // 设置当前文件总页数
