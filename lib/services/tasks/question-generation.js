  ...
  try {
    ...
    const processChunk = async chunk => {
      try {
        ...
        if (latestTask.status === 2) {
          throw new Error('任务已被标记为失败，停止处理');
        }

        const data = await questionService.generateQuestionsForChunk(task.projectId, chunk.id, {
          model: modelInfo,
    ...
    const finalNote =
      errorCount > 0
        ? `处理完成，成功: ${successCount}, 失败: ${errorCount}, 总问题数: ${totalQuestions}`
        : `处理成功，共生成问题: ${totalQuestions}`;
