...
import { processPdfProcessingTask } from './pdf-processing';
import './recovery';
  ...
  try {
    ...
    switch (task.taskType) {
      ...
      case 'text-processing':
        // 暂未实现
        console.log('文献处理任务暂未实现');
        await updateTask(taskId, { status: 2, note: '文献处理任务暂未实现' });
        break;
      ...
      case 'data-distillation':
        ...
        break;
      case 'pdf-processing':
        await processPdfProcessingTask(task);
        break;
      default:
        console.error(`未知任务类型: ${task.taskType}`);
        await updateTask(taskId, { status: 2, note: `未知任务类型: ${task.taskType}` });
    }
  } catch (error) {
    console.error(`处理任务失败: ${taskId}`, error);
    await updateTask(taskId, { status: 2, note: `处理失败: ${error.message}` });
  }
}
...
export async function updateTask(taskId, data) {
  try {
    ...
    if ((data.status === 1 || data.status === 2) && !data.endTime) {
