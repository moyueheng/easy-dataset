    ...
    const config = {
      ...
      onProgress: async ({ current, total, taskStatus }) => {
        if (updateTask && task.id) {
          taskMessage.courent.processedPage = current;
          taskMessage.courent.totalPage = total;
          taskMessage.setpInfo = `processing ${fileName} ${current}/${total} pages progress: ${(current / total) * 100}% `;
