...
export async function minerUProcessing(projectId, fileName, options = {}) {
  ...
  try {
    const { updateTask, task, taskMessage } = options;
    ...
    while (true) {
      try {
        ...
        taskMessage.courent.processedPage = currentPage;
        taskMessage.courent.totalPage = totalPage;
        taskMessage.stepInfo = `processing ${fileName} ${currentPage}/${totalPage} pages progress: ${(currentPage / totalPage) * 100}%`;

        //更新任务状态
        await updateTask(task.id, {
          totalCount: totalPage,
          completedCount: currentPage,
