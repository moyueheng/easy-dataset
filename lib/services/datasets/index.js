...
export async function generateDatasetForQuestion(projectId, questionId, options) {
  try {
    ...
    // 创建LLM客户端
    const llmClient = new LLMClient(model);    // 检查是否有可用的GA pairs (MGA功能)
    let activeGaPairs = [];
    ...
    if (chunk.fileId && !idDistill) {
      try {
        ...
        logger.info(`检查到 ${activeGaPairs.length} 个激活的GA pairs，${useEnhancedPrompt ? '使用' : '不使用'}增强提示词`);
      } catch (error) {
    ...
    if (idDistill) {
      // 对于精炼内容，直接使用问题
      prompt = question.question;    } else if (useEnhancedPrompt) {
      // 使用MGA增强提示词
      ...
      if (questionLinkedGaPair) {
        ...
        // 实现智能GA pair选择策略，确保多样性
        primaryGaPair = selectGaPairWithStrategy(formattedGaPairs, question.id, 'hash-based');
        logger.info(`使用智能选择的GA pair: ${primaryGaPair?.genre || 'None'} | ${primaryGaPair?.audience || 'None'}`);
      }
...
/**
 * 智能GA pair选择策略
 * @param {Array} gaPairs - 可用的GA pairs数组
 * @param {string} questionId - 问题ID，用于确保一致性
 * @param {string} strategy - 选择策略: 'round-robin', 'hash-based', 'random'
 * @returns {Object|null} 选中的GA pair
 */
function selectGaPairWithStrategy(gaPairs, questionId = null, strategy = 'hash-based') {
  if (!gaPairs || gaPairs.length === 0) {
    return null;
  }
  ...
  switch (strategy) {
    case 'round-robin':
      // 基于时间戳的轮换，确保在短时间内的分布
      selectedIndex = Math.floor(Date.now() / 1000) % gaPairs.length;
      break;
      
    case 'hash-based':
      // 基于问题ID的哈希，确保相同问题得到一致的GA pair
      if (questionId) {
        const hash = questionId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
        selectedIndex = hash % gaPairs.length;
      } else {
        selectedIndex = Date.now() % gaPairs.length;
      }
      break;
      
    case 'random':
      // 完全随机选择
      selectedIndex = Math.floor(Math.random() * gaPairs.length);
      break;
  ...
  const selected = gaPairs[selectedIndex];
  logger.info(`GA选择策略[${strategy}]: 从${gaPairs.length}个pairs中选择索引${selectedIndex} - ${selected.genre}`);
