...
export async function generateQuestionsForChunkWithGA(projectId, chunkId, options) {
  try {
    const { model, language = '中文', number, enableGaExpansion = false } = options;
    ...
    if (enableGaExpansion && chunk.fileId) {
      try {
    ...
    if (useGaExpansion) {
      ...
      logger.info(`GA扩展模式：将生成大于等于${baseQuestionNumber} 基础问题 × ${activeGaPairs.length} GA pairs = ${totalExpectedQuestions}个总问题`);
      ...
      for (const gaPair of activeGaPairs) {
        ...
        allGeneratedQuestions.push(...labelQuestions.map(q => ({
          ...q,
          gaPairId: gaPair.id,
