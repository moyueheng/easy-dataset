 ...
 */
export async function generateGaPairs(textContent, projectId, modelConfig = null, language = '中文') {
  try {
    ...
    // Prepare the prompt with text content - 修复作用域问题
    let prompt;
    if (language === '中文') {
      prompt = GA_GENERATION_PROMPT.replace('{text_content}', textContent);
    } else {
      prompt = GA_GENERATION_PROMPT_EN.replace('{text_content}', textContent);
    }
...
function parseGaResponse(response) {
  try {
    ...
    logger.debug('Raw LLM response:', response.substring(0, 500) + '...');
